...
de parada tomados usualmente es de\todo{sacar 'de'} detener la exploración cuando el mapa
construido alcanza cierto un porcentaje de cubrimiento del entorno
\cite{Yan2015}, auque\todo{. Aunque} este es usualmente utilizado para evaluaciones de
rendimiento no es útil en \todo{la}práctica, ya que para utilizarse se requiere tener
...
ninguna posibilidad de obtener nueva información de el\todo{él}, osea que no exista
ningún espacio desconocido que sea accesible. La desventaja de este criterio es
que se fuerza a una exploration\todo{exploración?} exhaustiva del entorno, lo cual puede ser
...
La definición $\mli{GVD}$ es análoga a su contra parte no generalizada y se define según se muestra en \eqref{eq:GVD}. 
...
% Los algoritmos que requieren recomputarse completamente al cambiar parte de la entrada son conocidos como \emph{algoritmos no incrementales}, por otro lado los algoritmos que actualizan la salida anterior a partir de los cambios experimentados desde la última entrada son llamados \emph{algoritmos incrementales}.
Los algoritmos que actualizan el resultado anterior a partir de los cambios experimentados son llamados \emph{algoritmos incrementales}, en consecuencia los que\todo{sacar 'que'} algoritmos que al cambiar parte de la entrada descartan el último resultado y recomputando\todo{tiempo verbal!} el nuevo resultado desde cero se denominan como \emph{algoritmos no incrementales}.

En la sección \ref{subsec:constGVD} el algoritmo presentado no es incremental, su uso no es adecuado cuando se desea mantener GVD asociado a un entorno actualizado en tiempo real mientras este es explorado, para esto lo ideal es utilizar un algoritmo incremental. A continuación se describe el algoritmo no incremental conocido como \emph{brushfire} y posteriormente la variante incremental de este denominada \emph{brushfire dinámico}.
...
En \ref{subsec:constGVD} se presenta un método de construcción que requiere primero encontrar un mapa de distancia y poseteriormente aplicar un algoritmo de esqueletización, brushfire logra construir el GVD evitando el paso intermedio de esqueletización. La idea se basa en asignar identificadores a cada conjunto conexo de celdas ocupadas, y luego según la interpretación de brushfire como ondas, las ondas provenientes de celdas ocupadas con identificadores distintos colisionan en las celdas que deben pertenecer al GVD. 
...
\todo{este párrafo y el sgte están enredados.  hay que amenizarlos.  escribir
  frases cortas y agregar alguna img que ayude a identificar las 2 situaciones
  descritas y los procedimientos de actualización de los valores de distancias.
  como la fig 10 pero sería ideal considerar cada caso por separado}

El algoritmo lleva a cabo las actualizaciones propagando los cambios ocurridos
