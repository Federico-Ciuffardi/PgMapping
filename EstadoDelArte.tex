...
Existen dos tipos principales de métodos para la construcción de un GVD los basados en sensores y los basados en grillas. En el contexto del trabajo desarrollado serán relevantes los métodos de construcción de un GVD basada en grillas. 

La idea más intuitiva para generar un GVD en un espacio representado con una grilla es la de adaptar la definición original de GVD que considera espacios continuos para considerar en su lugar espacios discretizados en celdas, y luego celda a celda, determinar según la nueva definición si estas pertenecen o no al GVD. La definición adaptada en lugar de puntos considera celdas las cuales pertenecen al GVD si tiene dos o mas celdas obstaculizadas a una misma distancia, siendo la distancia entre celdas la distancia entre sus centros. Esta técnica genera resultados erróneos al no considerar los efectos que causa la discretización. Por ejemplo en corredores cuya discretización tiene una cantidad par de celdas entre sus paredes, si se aplica directamente la definición de GVD a las celdas ninguna debería pertenecer al GVD dado que ninguna celda tendría una misma distancia a dos celdas obstaculizadas (paredes del corredor). Esto se considera incorrecto porque los puntos del centro de un corredor siempre pertenecen a un GVD construido en un espacio continuo. %El método de \cite{wurm2008coordinated} soluciona este tipo de problemas porque la esqueletización asegura la generación de puntos pertenecientes al GVD en las 

En \cite{wurm2008coordinated} se propone un método que resuelve dicho tipo de problemas. El método comienza aplicando una transformación de distancia Euclidiana \cite{meijster2002general} a una mapa métrico representado con una grilla donde cada celda esta ocupada o libre , resultando en un mapa que contiene para cada celda la distancia al obstáculo mas cercano, este tipo de mapa se denomina \emph{mapa de distancia} a los obstáculos. El grafo generalizado de Voronoi se obtiene aplicando una esqueletización \cite{zhang1984fast} que consiste en erosionar las celdas mas cercanas a los obstáculos hasta llegar a las celdas mas distantes a estos siendo estas ultimas las celdas que se asignan al GVD construido. Un ejemplo de este proceso se puede ver en la figura \ref{fig:wurmGVD}.
\begin{figure}[H]
  \centering
...
El nombre de este algoritmo proviene de como este va determinando las distancias, comenzando desde cada obstáculo se va extendiendo desde las celdas mas cercanas a las mas lejanas asemejándose a un incendio de maleza (traducido al ingles como brushfire). Comportamiento que también se puede interpretar como ondas que parten desde cada obstáculo, asignando la distancia recorrida a cada celda por la que pasan mientras avanzan.

Una observation sobre la interpretation de brushfire como ondas es que en los lugares donde estas colisionan la distancia minima hacia los obstáculos se maximiza

En \ref{subsec:constGVD} se muestra un método de construcción que requiere primero encontrar un mapa de distancia y luego aplicar un algoritmo de esqueletización, pero brushfire puede ser adaptado para construir el GVD sin el paso intermedio de esqueletización. Según la interpretación de brushfire como ondas se tiene que las celdas que pertenecen al GVD serán las que se 
