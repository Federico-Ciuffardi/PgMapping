...
La exploración multi-robot consiste en utilizar una flota robótica para obtener de información de un entorno desconocido con el propósito de generar un mapa que lo represente. 

El uso de varios robots es útil porque introduce redundancia, aumentando la tolerancia a fallos. Por otro lado la exploración suele ser altamente paralelizable ya que en un mismo instante de tiempo suelen haber varios lugares en los cuales un robot puede obtener información, por lo que aumentar la cantidad de recursos robotics es esperable que se reduzcan los tiempos de exploración\cite {cao1997cooperative}\cite{dudek1996taxonomy}\cite{guzzoni1997many}. Sin embargo aumentar el numero de robots en la flota de exploración, aumenta el riesgo de que estos se interfieran entre si, causando perdidas de tiempo en desvíos para evitar colisiones\cite{guzzoni1997many}\cite{goldberg1997interference}. Esto determina que existe un numero optimo de robots en una flota, en el cual se logra un balance entre la capacidad de explotar el paralelismo en la exploración y el tiempo perdido por interferencias. Un mecanismo de coordinación que logre evitar interferencias suele ser un factor critico en la exploración multirobot porque permite lograr el balance anteriormente mencionado con una cantidad de robots mayor al que se podría tener sin coordinar\cite{nieto2014coordination}.
...
\subsection{Mapas}\label{subsec:mapas}\cite{Liu2015}\cite{Thrun1998}
...
Una \emph{grilla de ocupación} es un mapa métrico que se basa en una teselación del espacio en celdas, donde cada celda almacena una estimación probabilística de su estado (libre, ocupado, etc). 

Los \emph{mapas topológicos} por otro lado son mapas simplificados al punto de solo incluir regiones topológicas y conexiones entre estas. Normalmente se representan con grafos donde las regiones de interés son los vertices y las aristas son las conexiones entre ellas.

Una gran parte de los mapas topológicos son \emph{mapas topológicos basados en regiones}, estos consisten en una segmentación del espacio en regiones de interés que serán las regiones topológicas. El concepto de region de interés es equivalente al de habitaciones y corredores en entornos estructurados como lo son las casas y oficinas. 
...
\begin{figure}[ht]
  ...
  \end{center}
  \caption{Ejemplo del método de generación de \cite{wurm2008coordinated}. A la izquierda esta el mapa original, en el centro el mapa original y su correspondiente mapa de distancia (cuanto más oscuro es un punto, mayor es la distancia al obstáculo más cercano). A la derecha se encuentra el mapa original junto a la esqueletización del mapa de distancia (lineas negras) que es a su vez el GVD construido. Extraído de \cite{wurm2008coordinated}.}\label{fig:wurmGVD}
\end{figure} 
...
% TODO describir bien el algoritmo no incremental de Lia ming
...
A lo largo de la exploración es usual que el mapa construido reciba de forma frecuente actualizaciones que solo modifiquen una pequeña porción del mismo. Dado esto por motivos de eficiencia  se busca que los algoritmos que utilizan al mapa como una entrada eviten recomputarse completamente en cada actualización y que en su lugar actualicen el ultimo resultado obtenido para que sea consistente con la ultima version del mapa.
...
\cite{kalra2009incremental}\cite{Lau2013}
...
Dado el problema de la coordinación para la exploración multirobot mencionado en \ref{subsec:expmutirob}, fueron desarrolladas varias tecnicas de coordinacion sur


Incluir aca a wurm la parte de coordinacion

\subsection{Coordinación a partir de segmentaciones no topológicas}
