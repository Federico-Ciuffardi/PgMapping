...
\todoerror[inline]{Vi la tendencia preferí no comentarlo porque no es muy relevante, y para lo relevante que es parece que es complicado de explicar. Trate de hacerlo pero si no quedo bien no creo que valga mucho la pena gastar más tiempo en esto | sobre el análisis de datos, algo que llama la atención -es una tendencia, es cómo incide positivamente tanto en calidad como en completitud, la granularidad de la grilla. desarrollaría un poco eso.}

\input{tablas/todo3}
\todo{mio: esta tabla tambien para arriba (luego de la ref)?}
...
\ref{fig:gra:inc:gvdt}\todo[inline]{referenciaría cada figura al final del párrafo que analiza el contenido asociado a ellas}.

En cada nivel de granularidad\todoerror[inline]{Entiendo, pero uso mucho el concepto de granularidad, en todos tengo que indicar que es la granularidad de la grilla? | de qué?} la variante incremental reduce en un
${\smallsim}94\%$ el tiempo promedio construcción del GVD con respecto a la
variante no incremental\todo{(Ver figura 4.7.)}. Esto valida la idea de que la construcción incremental
...
reducción de ${\smallsim}63\%$ en el nivel más alto\todo{(Ver figura 4.5.)}. Esto se explica porque el
...
aumentar los niveles de granularidad\todo{(Ver figura 4.6)}. Esto puede deberse a que tiempos más
...
métodos son: la simplificación\todoerror[inline]{ Lo que entendí de que decís (y estoy de acuerdo) es las fronteras como concepto no son más simples (no lo van a ser a no ser que cambie el mapa). La simplificación que digo yo va por el lado de la representación de ese concepto. 
...
\ref{fig:gra:idobj:iobt}\todo[inline]{ídem sec anterior}.
...
\ref{fig:gra:des:gvdt}\todo{ídem sec anterior}.
