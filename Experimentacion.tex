...
Completitud y error del mapa.

Tiempo de exploracion y costo de exploracion (suma de distancia recorrdia).

Tiempo de objtencion de info (suma)

Tiempo de contruiccion del GVD (suma)

Tiempo de identificación de objetivos(suma)

Porcentajes de contruccion del GVD y de identificación de objetivos.
...
esto se presume que la completitud de los mapas no es perfecta debido a la precencia de celdas
ubicadas en esquinas de paredes que que a pesar de no cumplir con las
codiciones de ser detectadas como explorables, lo son. Un ejempo de este tipo
de celda se muestra en la figura \ref{fig:faltaCub}. Sin embargo los valores de
completitud obtenidos indican que este tipo de situaciones son despreciables.
...
\begin{figure}[H]
  ...
  obtenido en las pruebas que presenta una celda explorable que no se reconoce
como tal. Ambos mapas corresponden a una granularidad de una celda por metro cuadrado.}\label{fig:faltaCub}
   % A la izquierda se muestra el robot real, y a la derecha su version simulada.

\end{figure}

Los resultados de calidad del mapa obtenidos son similares a los de
