% En este capítulo se presentan y analizan los resultados experimentales de la
...
A lo largo de esta se sección se especifican las pruebas realizadas en este proyecto.
 % y presentadas a lo largo de este capítulo.
...
% las pruebas que se presentan a lo largo del capítulo.

% En lo que resta de esta seccion se especifican las caracteristicas de las
% pruebas llavadas a cabo. 

% A lo largo del capítulo se presentan diversas pruebas que buscan comparar ciertos
...
% simplicar dichas fronteras, tanto la obtención de fronteras significativas basada en K-Means, como la
...
obtiene fronteras significativas basándose en el cubrimiento (sección
...
como objetivos (sección \ref{subsec:todasFront}), y solo identificar como objetivos a las
fronteras significativas obtenidas basandose en K-Means (sección
...
promedio en obtención de fronteras significativas}.
...
El \emph{tiempo promedio en obtención de fronteras significativas} es el tiempo promedio
que se demora en obtener las fronteras significativas en la etapa de obtención
de información de cada asignación de objetivos.
...
\subsection{Completitud y calidad de los mapas} \label{sec:exp:cubcal}
% Capaz aca puedo mencionar lo que pasa con los errores

% el cubrimiento y la completitud: completitud mayor a 0.999 para todas combinaciones de granularidad y soluciones probadas
...
\input{tablas/todo3}
% \todo{si  | esta tabla tambien para arriba (luego de la ref)?}
...
Aunque a pesar de esto la calidad de los mapas no es perfecta, esto puede explicarse en primer
...
% \todoerror[inline]{Vi la tendencia preferí no comentarlo porque no es muy relevante y parece que es complicado de explicar. Trate de hacerlo pero no creo que valga mucho la pena invertir más tiempo en esto. Quizás mejor no comentarlo. | sobre el análisis de datos, algo que llama la atención -es una tendencia, es cómo incide positivamente tanto en calidad como en completitud, la granularidad de la grilla. Desarrollaría un poco eso.}
...
% \todoremark[inline]{1: si, 2: poner figura luego del párrafo si es corto, y en el medio del párrafo si no referencia  | 1: Saco las referencias a las gráficas de acá? 2: Quedo bien? Sobre todo la de sección de identificación de objetivos | referenciaría cada figura al final del párrafo que analiza el contenido asociado a ellas}

\input{tablas/incrementalidad1}
% \todo[inline]{colocaría esta tabla inmediatamente a continuación de su referencia.}
% \todoerror[inline]{Aclarar la primera vez de la seccion, minimo (y en otrso parrofs si estasn muy alejados) | Entiendo, pero uso mucho el concepto de granularidad, en todos debería indicar que es la granularidad de la grilla? | de qué?} 
...
totales recorridas por la flota en todos niveles de granularidad de la grilla, aunque en este
...
métodos son: la obtención de fronteras significativas basada en cubrimiento propuesta en
este proyecto, la obtención de fronteras significativas basada en K-Means como es utilizada en
\cite{Amorin2019} y la identificación de todas las fronteras como objetivos
...
Es posible apreciar que los métodos que solo identifican a las fronteras significativas como objetivos 
reducen los tiempos de exploración con respecto al método que
identifica a todas las fronteras como objetivos. Aunque en el nivel
...
el tiempo de obtención de fronteras significativas y la cantidad de objetivos
identificados. Mayores tiempos de obtención de fronteras significativas retrasan la
...
en los resultados, es que invertir tiempo en disminuir los objetivos
identificados obteniendo las fronteras significativas lleva a una reducción total de los
tiempos de asignación de objetivos, y en consecuencia de los tiempos de exploración
frente a no invertir dicho tiempo y utilizar todas las fronteras como objetivos. Otra razón posible para las reducciones del tiempo de exploración
es que usar solo las fronteras significativas como objetivos puede aportar a la coordinación de los robots.
Por ejemplo de usarse todas las fronteras como objetivos se permite que un robot sea
...
capacidad de sensado, y se complejiza la navegación. Al usarse solo las
fronteras significativas como objetivos, especialmente cuando se logra el cubrimiento
con la menor cantidad de fronteras significativas posible, se evitan
situaciones como las descritas lo cual puede
verse como una coordinación pasiva. %\todo{o implicita?}

De acuerdo con los resultados usar solo las fronteras significativas como
objetivos también reduce las distancias totales recorridas por la flota frente
a usar a todas las fronteras. Estas reducciones son en general de ${\smallsim}10\%$ (ver figura \ref{fig:gra:idobj:ec}), y
...
Con relación a los dos métodos de obtención de fronteras significativas, en el
nivel de granularidad de la grilla más bajo, el método basado en cubrimiento
logra reducciones con respecto al basado en K-Means de ${\smallsim}3\%$ en las
distancias totales recorridas por la flota y de ${\smallsim}5\%$ en los tiempos
de exploración. Esto se invierte a medida que aumenta el nivel de granularidad,
hasta que en el nivel más alto en lugar de reducirse, las métricas aumentan.
Las distancias totales recorridas por la flota aumentan en un ${\smallsim}3\%$
y los tiempos de exploración en un ${\smallsim}6\%$ (ver figuras
\ref{fig:gra:idobj:et} y \ref{fig:gra:idobj:ec}). Por otro lado según los
tiempos promedio en obtención de fronteras significativas, el método basado en
cubrimiento en todo nivel de granularidad es entre $2$ y $4$ veces más lento
que el basado en K-Means (ver figura \ref{fig:gra:idobj:iobt}). Estos resultados
indican que la obtención de fronteras significativas basada en cubrimiento es
menos eficiente que la basada en K-Means en cuanto al tiempo de ejecución, a
pesar de esto no hay diferencias estadísticamente relevantes en los tiempos de
exploración ni en las distancias totales recorridas por la flota. Estos
resultados no permiten deducir de forma concreta cual método logra mejores
reducciones de objetivos.

% \todoerror[inline]{Corregir aca, en las conclusiones y en el trabajo futuro.
% Tengo info en las notas | Entiendo y estoy de acuerdo que la pista que hay en
% los datos actuales no da para asegurarlo. Pero la pista que hace en estos
% datos me parece que algo vale. De ser así como puedo manejarlo? Acá trate de
% hacerlo con el -parecen indicar- quizás hay una expresión mejor que permita
% hacer énfasis en que es solo una posible explicación para esos resultados (la
% que hago en el siguiente párrafo).  | mmmm no. los resultados muestran que
% k-means es más rápido y que supera a la nuestra cuando aumenta la
% granularidad considerando distancia y tiempo medio de exp. para decir que las
% reducciones son mejores, habría que p.e. contar la cantidad de objetivos que
% se generan a lo largo de la exp con una u otra estrategia. eso daría una
% pista certera sobre el impacto de seguir una u otra y de allí se le podría
% atribuir que la nuestra distribuye mejor o hace mejores reducciones.}. En el
% nivel de granularidad más bajo el aumento de tiempo de obtención de fronteras significativas no es
% tan relevante como la reducción de objetivos, por lo tanto la obtención de fronteras significativas
% basada en cubrimiento obtiene un mejor rendimiento. A medida la granularidad
% crece el aumento de tiempo de obtención de fronteras significativas se torna más relevante que la
% reducción de objetivos, por lo que la obtención de fronteras significativas basada en K-Means
...
% Construir el GVD de forma incremental también parece reducir las distancias
% totales recorridas por la flota en todas las granularidad, aunque en este
...
%%, cuando los tiempos de obtención de fronteras significativas son los más cortos,
%%

  % Dado que la
% obtención de fronteras significativas basada en cubrimiento tiene siempre un mayor costo temporal que
...
% Con respecto a las dos tecnicas de obtención de fronteras significativas estas tienen comportamientos
% similares tanto en el tiempo de exploración como en distancia total recorrida
% por la flota. En el nivel de granularidad más bajo, la obtención de fronteras significativas basada en
...
% que ocurre en los niveles de granularidad más altos, donde la obtención de fronteras significativas
...
% obtención de fronteras significativas. 
...
% demora la obtención de fronteras significativas. 

% Dado esto se pasa a
% considerar otro factor de impacto de los tiempos de asignación: el tiempo que
% demora la obtención de fronteras significativas. Según los resultados el tiemp

% Dado que estos metodos
% solo difieren en como realizan la obtención de fronteras significativas, la demora de
% dicha obtención de fronteras significativas es uno de los principales factores a tener en cuenta.
...
% tiempo de obtención de fronteras significativas es en realidad una inversión de tiempo que se hace
...
% obtención de fronteras significativas, ya que estos la cantidad de objetivos para disminuir el
% tiempo total de la asignación de objetivos.

% se mantienen entre un ${\smallsim}9\%$ y ${\smallsim}11\%$, exceptuando la
% granularidad de una celda por metro cuadrado en la obtención de fronteras significativas basada en
% K-Means, en la que se obtiene una reduccion del ${\smallsim}7\%$

% Con respecto a las dos tecnicas de obtención de fronteras significativas los comportamientos en las
...
% obtención de fronteras significativas basada en el cubrimiento es la mejor cuando la carga
% computacional es baja, siendo superada por la obtención de fronteras significativas basada en K-Means cuando
