...
\begin{figure}[H]
  ...
  \caption[Proceso de simplificacion de fronteras según K-Means.]{Proceso de simplificacion de fronteras según K-Means. Las circunferencias rojas indican el cubrimiento de cada frontera significativa. \cite{Amorin2019}.}\label{fig:ejemploFrontSig}
...
\begin{figure}[H]
  ...
  \caption[Simplificacion de fronteras suboptima resultante del metodo basado en K-Means.]{Simplificacion de fronteras suboptima resultante del metodo basado en K-Means. Extraída de implementacion desarrollada\footnotemark.}\label{fig:ejemploFSKMMal}
\end{figure}
...
\begin{figure}[H]
  ...
  \subfloat[Resultado final mostando circulos de $radio=rango$ centrados en cada frontera significativa.]{\includegraphics[clip=true, width=0.40\textwidth]{imagenes/ejemploSimpCub/zfinal2.png}}
  ...
  orden en la cola, y las fronteras significativas con verde.}\label{fig:ejemploFSCub}
...
Como se menciona en \ref{sec:exploracion} la asignación de tareas se puede
dividir en dos partes, la identificion de objetivos y la asignación de objtetivos.

La identificacion se lleva a cabo con el metodo descrito en la sección
\ref{subsec:MiSimp}.
...
las regiones (tambien llamados segmentos) de un mapa topológico (seccion \ref{subsec:mapas}).

Para lograr las distribucion de los robots en segmentos es necesario un mapa
topológico, que permita dentificar en que segmento se encuentra cada objetivo
de exploracion. El mapa topológico se construye con la tecnica descrita en la seccion
...
alcanzar una implementación que logre funcionar en tiempo real
(seccion \ref{subsec:constGVDInc}) se reemplaza por una variante del algoritmo
incremental \emph{brushfire dinamico} que se presentará en la seccion
\ref{sec:MiConstGVD}.

La asignación se lleva a cabo organizando desde la central una subasta donde
los objetivos son subastados entre los robots. La subasta se puede separar en
dos partes, la valuacion y la resolucion. La valuacion consite en la
distribuicion de los objetivos de exploracion desde la central hacia los
robots, la asignación de valores que cada robot debe hacer a cada objetivo y el
posterior envio de estas valoraciones desde los robots hacia la central. La
resolucion contempla la recepcion de las valoraciones por la central, la
asignacion de objetivos a robots segun y la notificacion del objtivo asignado a
los robots. 
...
La resolucion es el proceso de asignar los objetivos de exploracion a los
robots. 


La version original de \cite{wurm2008coordinated} lleva a cabo la
resulucion utilizando el metodo hungaro. Este logra asignar los robots de 
...
Los objetivos de exploracion a subastar el artículo propone priorizar la distribución de los robots y no simplemente realizar un enfoque basado en las fronteras. Recordar que la segmentación busca que los robots se separen y exploren las habitaciones, aún cuando tengan fronteras cercanas a ellos en habitaciones exploradas por otros robots.

La central da comienzo a la subasta enviando a cada robot el mapa global hasta el momento, el GVD e información sobre los segmentos. De cada segmento se envía, el punto crítico que delimita el segmento y las fronteras del mismo, ordenadas en forma creciente por distancia al punto crítico.
...
Recordar que los puntos críticos suelen posicionarse en puertas (mínimos locales, grado 2, vecino de una intersección), por lo que son entradas a habitaciones. En el caso de que este pertenezca al camino robot-frontera significa que el robot se encuentra fuera de la habitación y por ende se considera fuera del segmento. Asimismo, la frontera más cercana al robot será la frontera más cercana al punto crítico. 
La aplicación de este factor de descuento resulta en que aquellos robots que tengan una distancia mayor a la puerta (punto crítico) y que consecuentemente se encuentren más adentrados en el segmento, sean priorizados para que continúen la exploración en este, a diferencia con aquellos que se encuentran más cercanos a la puerta y el costo de cambiar de segmento sea menor.

Para unir un nuevo punto al GVD es utilizada una función de distancias similar a la utilizada para la generación de GVD. Consta en expandirse desde la celda hasta llegar a un vértice del GVD. Esta celda pertenecerá al mismo segmento que el vértice más cercano a ella.

Continuando con la subasta, el robot prepara la oferta y la envía a la central, quien estaba esperando por estas. En la sección  4.5, Ventanas, se discute el tiempo que la central debe esperar por estas respuestas. Más aún, cada subasta tiene su propia identificación, evitando que se mezclen mensajes de subastas anteriores. 
...
En resumen, la central asigna a los robots de manera   
greedy, priorizando distribuir equitativamente a los mismos entre los segmentos. La central informa sobre las asignaciones a los robots y queda a la espera de un pedido por parte de alguno de los robots para volver a comenzar otra subasta.


\section{Contruccion del GVD}\label{sec:MiConstGVD}
...
Aunque existe una etapa de erosion posterior.

Oportunidad de comentra que mejora sobre lau porque perminte generar gvd en
lugares a dist 1 de los obstaculos, cosa que lau no permite ni explica porque.
Experimentalmetne sospecho que tiene problemas de desconexion. 

Desribir problemas de desconexion.

Mostrar que liu ming tambien los tiene???
