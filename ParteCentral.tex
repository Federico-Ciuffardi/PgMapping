...
\subsection{Simplificacion de fronteras basada en cubrimiento}
...
\SetAlgoLined
  ...
  \While{$\mli{UC} \neq \emptyset$ } {
    ...
    \If{ $\mli{pf} \in \mli{UC}$}{
      ...
      $\mli{FSC} :=$ celdas en $\mli{UC}$ que solapen con $\mathscr{C}(\mli{pf},radio)$\\
...
de celdas en $\mli{UC}$ que definen la circunferencia discretizada %\cite{foleyphillips}
...
Asignación estaria buenisimo comentar sobre todo porque Wurm aparentemente no
...
\subsection{Contruccion del GVD}\label{sec:MiConstGVD}

\subsection{Segmentacion}
Creo que aca en realidad no habria que comentar mucho, mismo las ideas son las comentadas en Thrun.
