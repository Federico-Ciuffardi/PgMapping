...
% Propuesta  de un algoritmo de construcción incremental de un GVD basado en \emph{brushfire dinámico} según se describe en \cite{Lau2013} cambiando principalmente la forma de determinar las celdas pertenecientes al GVD.
...
% Y que adicionalmente se implemento una construiccion de GVD novedosa que basa su construiccion en sources y pseudosources (la idea intuitiva pero considerando los potenciales problemas de discretizacion)
...
Durante la exploracion un subconjunto de celdas de la grilla de ocupacion usada
para representar el entrono explorado, cuyo estado se determina como
desconocido. Para una celda $c$ con estado desconocido que $c$ este ocupada y
que $c$ este libre, son sucesos equiprobables, siendo necesaria evidencia de
que la celda esta ocupada ni de que esta libre, para que la celda cambie su
estado ya sea a libre o a ocupado. En un escenario donde comienza sin conocimiento sobre
...
A pesar de esto, las celdas de estado desconocido no es un tema que se trate de
forma explicita los trabajos estudiados sobre la contruccion incremental de
GVD. A pesar de esto a partir del codigo e imagenes disponibles es posible
deducir que en \cite{kalra2009incremental} y \cite{Lau2013} las celdas de
estado desconocido se consideran de igual forma que las de estado libre al
construir el GVD. De esta forma se obtienen resultados similares a los que se
presentan en la figura \ref{fig:kalraOG}. Notar que otra caracteristica
particular presente en estas implementaciones es que las celdas ubicada en los
bordes del entrono se consideran como pertenecientes a generdores.
...
desconocido y generar la pertenecia de la celdas desconocidas al GVD. Esto
tiene asociado un costo computacional, que es relevante dado que como ya se
menciono una gran parte del entorno es desconocido en un principio.
...
Al recibir un objetivo de exploracion, desde la central (seccion
\ref{sec:asigTar}), el robot debe moverse fisicamente hasta el lugar indicado.

Cuando el modulo \emph{controlador del robot} recibe el objetivo, este recupera de la
memoria del robot el camino que lleva al objetivo que fue determinado para
valuar dicho objetivo (seccion \ref{subsec:MiValSub}) y transmite dicho camino
al modulo \emph{controlador de movimiento}. 

El camino consiste en una secuencia de centros de celdas adyacentes entre si,
que comienza en el centro de celda mas cercano a la posición actual del robot y
termina en el centro de celda correspondietne al objetivo. Los puntos del
camino se denominan subobjetivos con exepcion del ultimo que es el objetivo.

El modulo \emph{controlador de movimiento} se encarga de enviar el primer
objetivo/subobjetivo del camino que se considera no completado al modulo \emph{move
base} el cual se encarga de generar las directivas a los motores del robot para
que pueda llegar de forma segura a dicho objetivo/subobjetivo. En algun punto de la
trayectoria hacia el objetivo/subobjetivo que lleva a cabo \emph{move base}
este se considera completado, dado esto el siguiente objetivo/subobjetivo
sin completar se envia a \emph{move base}. El proceso se repite hasta completar
el objetivo, completando asi el camnino completo.

El \emph{controlador de movimiento} tiene dos formas de considerar completo un
objetivo/subobjetivo, la normal y la forzosa.

La compleción forzosa es igual en objetivos y subobjetivos ocurriendo cuando el
robot esta a una distancia menor que $\mli{DistF}$ de estos. Este criterio de
compleción tiene como proposito determinar cuando un robot esta lo
suficientemente cerca de un objetivo/subobjetivo como para considerarlo
completo a pesar de que no se dio aun la complecion normal.

Por otro lado la compleción normal se da cuando el segmento de recta que
existe entre el robot y el objetivo/subobjetivo no contiene obstaculos. Adicionalmente, 
en el caso de un subobjetivo la distancia entre este y el robot debe ser menor
a $\mli{DistNs}$ y en el caso de un objetivo desde este al robot debe ser menor a
$\mli{DistNo}$.

El valor de $\mli{DistF}$ se establece para cumplir con el proposito de la complecion
forzosa, para esto debe considerarse las dimenciones del robot, el tamaño de la
grilla entre otros factores. 

EL valor $\mli{DistNo}$ debe cumplir con $\mli{DistF}<\mli{DistNo}<\mli{rango}$, siendo $\mli{rango}$ el
alcance de los sensores del robot. Esto se debe a que la idea detras de
criterio de complecion normal asociado al objetivo es considerar que el
objetivo final se completa cuando el robot tiene una linea directa de vision y
el objetivo esta en el rango de sensado del robot ya que esto implica que el
robot esta en condiciones de recopilar nueva información del entorno, que es el
motivo por el cual el objtivo fue asignado en un primer lugar.

Por otro lado el valor $\mli{DistNs}$ solo debe ser mayor que $\mli{DistF}$, y
establece que tan estrictamente se sigue el camino original en espacios
despejados de obstaculos. Valores pequeños de $\mli{DistNs}$ hacen que el robot
siga el camnino original de foma estricta aunque este en espacios despejados ya
que fuerzan a que el robot este cerca de los subobjetivos para completarlos. Al
...
espacios despejados todos los subobjetivos a una distancia menor a
$\mli{DistNs}$ se consideran completados, siendo move base el encargado de la
trayectoria hasta el primer subobjetivo no aun no completado, sin estar
obligado a pasar cerca por los subobjetivos previamente completados.

Valores de $\mli{DistNs}$ similares al de las dimenciones de los segmentos de
un entorno estructurado llevan a una navegacion jerárquica. Esto sucede porque
asumiendo segmentos despejados, un robots completa todos los subobjetivos
asociados a un segmento cuando se encuentra en su interior. Por lo tanto en
en caminos largos que abarcan varios segmentos, los unicos subobjetivos
enviados a \emph{move base} son los que se ubican en las transiciones entre un
segmento y el siguiente segmento en el camino. Es decir que el robot utliza el
camnino original (en su mayor parte compuesto por centros de celdas
pertencientes a GVD) para determinar los subobjetivos que permiten transicionar
entre los segmentos del camino y \emph{move base} para navegar dentro de los
segmentos hacia dichos subobjetivos. La ventaja de esta navegacion jerárquica
es que es eficiente calcular camnios sobre el GVD, pero los caminos resultantes
pueden ser innecesariamente largos,  
