...
\mli{DBP}_{c_1}$ como es deseado, ya que recordando que $b_2\in \mli{CGen}$ entonces:
...
\subsection{Criterio de pertenecia}

Dado un choque de onda que involucra dos celdas $c_1$ y $c_2$, en
\emph{brushfire dinamico} original escencialmente estas pertenecen al GVD si
$g_1$ y $g_2$, sus respectivas celdas más cercanas que pertenecen a un
generador son distintas y no adyacentes entre si. 

La defincion de GVD para el espacio continuo donde un punto $p$ pertenece al
GVD si tiene dos generadores distintos a minima distancia, lo cual implica que
$p$ tiene al menos un punto de cada uno de esos generadores a minima distancia,
estos puntos son parte de los puntos base de $p$ (seccion
\ref{subsec:mapaTopGVDGrid}). Por lo que se puede decir que un punto pertenece
al GVD si tiene al menos dos puntos bases que pertenecen a generdores
distintos. Al pasar a un espacio discretizado en celdas, estos implica que
existen dos celdas base que pertenecen a dos generadores distintos.

Dado este analisis y la necesidad de determinar las celdas base planteada en la
seccion \ref{subsec:mapaTopGVDGrid}, se determinó que la pertenencia de una
celda al GVD se establece segun la existencia de dos celdas base pertenecientes
a generadores distintos.
...
el \emph{brushfire dinamico} original. Cuando, durante el calculo del mapa de
distancia de un incremento se registran todas las celdas involucradas en algun
choque de olas. Al finalizar el calculo del mapa de distnacia siendo este
consistente con el ultimo mapa disponibles, se calculan las celdas base de
cada celda registrada previamente segun el algorimo X.
...
Teniendo las celdas base de un punto para determinar su pertenencia al GVD
resta verificar si existen al menos dos celdas base que pertenecen distintos
generadores. Para esto se aplica una tecnica derivada del criterio de pertenencia
de celdas al GVD en el \emph{brushfire dinamico} original, que consiste en que
dos puntos base distintos y no adyacentes entre sí pertenecen a distintos
generadores, y de lo contrario pertenecen al mismo generador.
De esta forma se logra aproximar el GVD resultante de considerar generadores
conexos y convexos \ref{subsec:GVD} sin definir explicitamente los generadores.
Y adicionalemtente se obtienen las celdas base a partir necesarias para
segmentar.
