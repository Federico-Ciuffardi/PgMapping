En este capitulo\todo{revisar ortografía!} se describen los principales aspectos de la solución al
...
Estas hipótesis tienen el motivo de\todo{procuran/tienen por objeto?} simplificar algunos aspectos del
...
que hacer consideraciones especiales para cada robot. La (III)\todo{parece que II y III quedaron cambiadas} por no tener que
...
el cual es posible ver los componentes, sus módulos y como\todo{cómo} estos se distribuyen
...
\end{figure} 
\todo[inline]{fig3.1: en el caption solución. colorear lo hecho con otro color}
...
seguirse en secuencia\todo{que al seguirse?} llevan al objetivo .
...
algoritmo para determinar que\todo{qué} objetivo le corresponde a cada robot. Finalmente
...
robot} si se completo\todo{completó} el camino con éxito, o si existe algún problema.
...
que resta del capitulo\todo{capítulo}.
...
Se dice que cada celda $c\in \mli{CG}$ tiene asociada una probabilidad $P(c|x_{1:t},z_{1:t})$\todo{$c_{t+1}$ ? explicitaría que t es el tiempo y que $1:t$ representa el conjunto de instantes de tiempo considerados}
...
\begin{equation} 
  ...
  \label{eq:estado}
\end{equation}
\todo[inline]{habría que definir un $\epsilon$ de modo que $|P-0,5|<\epsilon$ para desconocido y lo propio para las demás. sino, pareciera que en la práctica nunca serían desconocidas, no?}
...
\cite{hopcroft1973algorithm}\todo{daría una pista sobre cuál sea la función del alg en el diseño de la solución.}.
...
\end{figure}
\todo[inline]{fig3.5: ... fronteras utilizando K-Means.? otra, ... tienen radio=4 celdas, indicando el cubrimiento de un sensor de rango equivalente. ?}
...
soluciones optimas\todo{óptimas} reducen al mínimo el $\mli{FS}$ resultante, mientras
...
sección no es optimo\todo{óptimo} ya que existen fronteras significativas innecesarias para
el cubrimiento. 
...
\end{figure}
\todo[inline]{fig3.6: ídem comentario fig.3.5}
...
sobre los segmentos (habitaciones y corredores) que componen a\todo{sacar a} un entorno
estructurado.
% , que
...
información} es la etapa inicial donde \todo{la central obtiene la información ...}central obtiene información necesaria
...
Que las las\todo{sacar un las} celdas desconocidas se consideren iguales a las celdas libres
...
punto del camino fue completado\todo{alcanzado?}, el criterio de compleción normal y criterio de
compleción forzosa.
...
del camino como para considerarlo completo\todo{que se ha alcanzado el objetivo?}, a pesar de que no se dio aún la
...
sobre el GVD es más computacionalmente\todo{computacionalmente más?} eficiente que generarlos sobre la grilla
...
usados\todo{justificaría un poco más el uso de esos valores. por qué no 0.0 y 1.0, p.e.}. 
