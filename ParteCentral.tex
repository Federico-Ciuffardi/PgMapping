...
\emph{brushfire dinamico} como la funcion de despeje discretizada $\mli{DD} : C
...
\begin{equation}
\begin{split}
  \mli{CrD} = \{c\ \in C_{\mli{GVD}} : & \forall c_l \in ady(c) \cup \mli{GVD}, D(c) \leq D(c_l),\\
                                       & \exists c_g \in ady(c) \cup \mli{GVD}, (c_g) > D(c) \} %, \\
                                       % & |ady(c)\cup \mli{GVD}| = 2\}
\end{split}
\label{eq:critDisc}
\end{equation}

Al adaptar a celdas el concepto de punto critico, es conveniente no solo
considerar las celdas del GVD que minimizan el despeje localmente, es decir las
tienen un despeje menor que el de sus celdas adyacentes en el GVD, si no que
tambien incluir a las celdas que tienen un despeje menor o igual que el de sus
...
presentadas en la figura \ref{fig:celYlinCrit}, donde se puede observar un
...
concepto de puntos base de un punto, que refiere a los puntos mas cercanos de
los generadores. En \emph{brushfire dinamico} no se manejan los generadores de
forma directa, por lo cual para llegar a una defición celdas base (puntos base
discretizados), es necesario analizar el funcionamiento del algorimo.

En \emph{brushfire dinamico} cuando ocurre un choque de ondas consistentes en a
las celdas $c_1$ y $c_2$, sean $oc_1$ y $oc_2$ respectivamente sus celdas
ocupadas más cercanas se considera que si $oc_1$ y $oc_2$ son adyacentes
entonces pertenecen al mismo generador y de lo contrario que pertenecen a
distintos generadores. De esto se deriva que si $oc_1$ y $oc_2$ no son
adyacentes entoces $c_1$ y $c_2$ deben pertenecer al GVD. De esta forma se
logra aproximar el GVD resultante de considerar generadores conexos y convexos
\ref{subsec:GVD}. Analizando esto, se decidio definir los puntos que base
discretizados $\mli{BPD}_c$ se definen como las celdas obstaculizadas no
adyacentes que generan la pertenecia de una celda $c$ al GVD. Las lineas
criticas discretizadas se definen como las lineas discretizadas
...
\begin{figure}[H]
  ...
  \subfloat[Grilla de ocupacion]{\includegraphics[clip=true, width=0.40\textwidth]{imagenes/GVDDisc/a.png}}
  \qquad
  \subfloat[GVD]{\includegraphics[clip=true, width=0.40\textwidth]{imagenes/GVDDisc/b.png}}

  \subfloat[Celdas criticas contienen cuadrados amarillos y las pertinentes a lineas criticas discretizadas contienen cuadrados grises]{\includegraphics[clip=true, width=0.40\textwidth]{imagenes/GVDDisc/c.png}\label{fig:celYlinCrit}}
  \qquad
  \subfloat[Segmentación del entorno, cada segmento se indica con un color distinto]{\includegraphics[clip=true, width=0.40\textwidth]{imagenes/GVDDisc/d.png}}

  \caption{Proceso de segmentación en una grilla a partir de un
  GVD}\label{fig:ejSegGVDGrid}
