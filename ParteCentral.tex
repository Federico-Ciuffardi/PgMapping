...
\subsection{Simplificacion de fronteras basada en el cubrimiento}
...
alejadas posible de esta, es decir, que $\mli{FSC}$ %$= \{f\in \mli{UC} : d_{\mli{fs}}(\mli{fp})= rango$\}
...
de $F_i$ a $\mli{FP}$. El otro caso es el que se presenta si $\mli{FSC} = \emptyset$\todo{esto esta mal, en realidad esta mal desde antes cuando se presenta FSC, FSC busca que las candidatas queden centradas. Por lo que el radio puede ser menor al rango aunque haya una (o mas) celda solapada conla circunferencia de radio rango}, es
...
\SetAlgoLined
  ...
  \While{ $\neg \mli{UC}.vacia()$ } {
    ...
    \If{ $\mli{fp} \in \mli{UC}$}{
      $radio := max\{ d_{\mli{fp}}(\mli{f}) : f\in\mli{UC} \}/2$\\
      $radio := min(radio,rango)$\\
      $\mli{FSC} :=$ celdas en $\mli{UC}$ que solapen con $\mathscr{C}(\mli{fp},radio)$\\
      ...
      \ForEach { $\mli{fp} \in \{ f \in \mli{UF} : \exists c \in \mli{RC}, f \in ady(c)\}$}{
  ...
  \caption{Simplificacion de fronteras basada en el cubrimiento}
  \label{alg:IdObjGeo}
\end{algorithm}
