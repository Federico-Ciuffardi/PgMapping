...
generadores. Y adicionalemtente se obtienen las celdas base a partir necesarias
para segmentar.
...
En este contexto el considerar las celdas de estado desconocido se vuelve
importante ya que en un inicio estas ocupan un gran procentaje del total de las
celdas de la grilla.

En \cite{kalra2009incremental} y \cite{Lau2013} se considera que el espacio
desconocido es atravezable por las ondas consistentes e inconsistentes. A su
vez los bordes del mapa se rellenan con obstaculos. De esta forma se obtienen
resultados similares a los que se presentan en la figura \ref{fig:kalraOG}.
...
\begin{figure}[H]
  ...
  \caption[Consideración del espacio desconocido en \cite{kalra2009incremental} y \cite{Lau2013}.]{Consideración del espacio desconocido en \cite{kalra2009incremental} y \cite{Lau2013}. En la parte superior se muestra la grilla de ocupacion actual. En la inferior su GVD correspondiente, indicandose con rojo los obstaculos. Extraída de \cite{kalra2009incremental}.}\label{fig:kalraOG}
\end{figure}

El que las olas se propaguen por el espacio desconocido tiene una desventaja
asociada que es el costo computacion que implica atravezar y generar el GVD
sobre las partes desconocidas del espacio. Costo especialmente alto al comienzo
de la ejecucion cuando, como ya se menciono una gran parte del entorno es
desconocido. Esto se puede evidenciar en la figura X donde solo una pequeña
porcion del espacio es conocida y el GVD se extiende por todo el entorno.

Dado que en el contexto del proyecto no es necesario construir GVD fuera del
area desconocida se desarrollo una tecnica permite mantener un GVD conexo y
solo considerar el espacio conocido. Esta consiste en no permitir que las olas
atraviesen el espacio desconocido y en considerar como celdas pertenecientes a
un generador tanto a las celdas ocupadas como a las celdas desconocidas que
tienen un vecino conocido. De esta forma para el caso de la figura X se obtiene
un GVD reducido al espacio conocido, y se evita propagar las olas sobre el
espacio desconocido, como se muestra en la figura Y.

FIG X y FIGY


Una alternativa que resulto intuitiva y tuvo que ser descartada es la de
simplemente considerar el espacio desconocido como no atravezable, el problema
con esta es que resulta en GVD disconexos en algunos casos donde esto no es
necesario. Los GVD disconexos se buscan evitar ya que no representan un
\emph{roadmap} (seccion \ref{subsec:mapacarr}) al no cumplir con la propiedad
de \emph{conectividad}. En la figura \ref{fig:desconocidoEj} se muestra un GVD
generado con las tres formas de considerar el espacio desconocido en una misma
grilla de ocupacion, 
...
\begin{figure}[H]
  ...
  \subfloat[Lo desconocido no es atravezable por ondas y no genera ondas.]{\includegraphics[clip=true, width=0.32\textwidth]{imagenes/desconocidoCons/noHacerNada.png}}
  \quad
  \subfloat[Lo desconocido es atravezable por ondas y no genera ondas.]{\includegraphics[clip=true, width=0.32\textwidth]{imagenes/desconocidoCons/kalra.png}}
  \quad
  \subfloat[Lo desconocido no es atravezable por ondas y genera ondas.]{\includegraphics[clip=true, width=0.32\textwidth]{imagenes/desconocidoCons/mio.png}}

  \caption{Diferentes formas de considerar el espacio desconocido. El GVD generado se indica en azul.}\label{fig:desconocidoEj}
\end{figure}
