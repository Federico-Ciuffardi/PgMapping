...
\SetAlgoLined
  ...
  \ForEach { $f\in F_i$ } {
    ...
    \ForEach{ $\mli{fa} \in ady(f)$ } {
      \If { $\lnot encolada$ $\land $ $e(\mli{fa}) = ocupado$ } {
        $\mli{PF}.encolar(f)$\\
        $encolada$ $:=$ $verdadero$
      }
    }
...
% cubra una frontera $\mli{pf}$ desencolada de $\mli{PF}$ que todavía
% no este cubierta. Esto se traduce a la restricción $d_{\mli{pf}}(\mli{fs})
% \leq rango$. Adicionalmente se tiene una heurística que establece que dado
% $dCen = max\{ d_{\mli{pf}}(\mli{f}) : f\in\mli{UF} \}/2$, las fronteras que
% estén a $min \{dCen, rango\}$ de distancia de $\mli{pf}$ serán las mejores
...
% \{dCen, rango\}$ centrada en $\mli{PF}$ denotada como
