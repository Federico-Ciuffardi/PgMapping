...
Estas hipótesis tienen el propósito\todoremark{o capaz: se hacen con el motivo? |
procuran / tienen por objeto?} de simplificar algunos aspectos del problema de
...
\begin{figure}[H]
  ...
  \caption[Arquitectura de la solucion propuesta.]{Arquitectura de la solución propuesta. Los módulos coloreados en azul fueron implementados en este proyecto.}
...
Se dice que cada celda $c\in \mli{CG}$ tiene asociada una probabilidad $P(c|x_{1:t},z_{1:t})$\todoerror[inline]{Me parece que lo mejor es dejarlo así, es de esta manera como se usa en el libro de Stachniss | $c_{t+1}$ ? explicitaría que t es el tiempo y que $1:t$ representa el conjunto de instantes de tiempo considerados}
...
\begin{equation} 
  ...
  \label{eq:estado}
\end{equation}
\todoerror[inline]{Creo que no usar el epsilon es lo que más se ajusta a la
...
punto del camino fue completado\todoerror[inline]{Creo que es mejor el termino completado porque alcanzar me da una idea de cercanía que no es necesariamente el caso. Entiendo que quizás queda raro hablar de completar un punto del camino, pero esto se aclara a lo largo de la sección (y era el termino que se estaba manejando antes de comenzar la subsection) | alcanzado?}, el criterio de compleción normal y criterio de
compleción forzosa.
...
del camino como para considerarlo completo\todoerror[inline]{similar a lo anterior, aunque acá si esta lo de cercanía, me parece mejor completar para ser consistente | que se ha alcanzado el objetivo?}, a pesar de que no se dio aún la
...
usados.\todoerror[inline]{El libro de Stachniss no especifica más que $0 \leq pfree < pprior < pocc \leq 1$ y que $pprior$ suele usar como $0.5$. Y mismo los $\leq$ son cuestionables, pro ejemplo si $pfree = 0$ en la regla de update queda una division entre $0$. La verdad es que los valores principalmente los determine de forma experimental (mi respuesta a esas preguntas sería que 0 y 1 no funcionaron bien). La intuición que me quedo de las pruebas que hice para determinar esos valores es que valores de $pfree$ y $pocc$ más cercanos al $0.5$ logran que la historia de evidencia tenga más peso y los más alejados de $0.5$ que la evidencia reciente sea la que tenga más peso. Poner la intuición sin fundamento real no creo que este bueno, y buscar más por alguna fuente con una expedición o probar la intuición matemáticamente no creo que valga la pena. De última puedo aclarar que los valores se determinaron de forma experimental. | justificaría un poco más el uso de esos valores. por qué no 0.0 y 1.0, p.e.}
