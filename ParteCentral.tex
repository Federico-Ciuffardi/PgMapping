...
$\mli{DD} : \mli{CG} \rightarrow R$ que se corresponde con un mapa de distancia
...
\begin{equation}
\begin{split}
  \mli{CrD} = \{c\ \in \mli{GGVD} : & \forall c_l \in ady(c) \cap \mli{GVD}, D(c) \leq D(c_l),\\
                                       & \exists c_g \in ady(c) \cap \mli{GVD}, (c_g) > D(c) \} %, \\
                                       % & |ady(c)\cup \mli{GVD}| = 2\}
\end{split}
\label{eq:critDisc}
\end{equation}
...
Para adaptar el concepto de las lineas criticas es necesario adaptar
primero el concepto de puntos base de un punto $p$, que refieren al conjunto de
puntos pertenecientes a generadores que son mas cercanos a $p$. Las celdas base
(puntos base discretizados) de un a celda $c$ se definen como el conjunto de
celdas pertenecientes a generadores (notar que no es relevante saber a que
generdor pertnece cada punto) que son mas cercanas a $c$ \todo{explicar
problemas de dicretizacion (ming-siegwart)}. Luego las lineas criticas
discretizadas se definen como las lineas discretizadas \cite{foleyphillips}
definidas entre los centros de las celdas criticas y cada uno de sus puntos
base discritizados.
...
(I) permite comentar el tema de dist 1.
