...
El conjunto $\mli{CG}\subseteq \mathds{R}^2$ esta compuesto por los centros de cada
...
cada frontera esta dentro del rango del sensado desde alguna frontera
...
Una vez determinados los costos $c_{i,j}$ se ejecuta un algoritmo que resulta
...
segmentos del entorno. Este algoritmo de asignación tiene dos ventajas respecto al
...
es que resulta en una distribución de los robots en los segmentos que considera
...
hacer $N$ rondas en las que se asigna un solo robot a cada segmento que contenga
objetivos sin asignar. La ronda $N$ termina cuando no quedan robots u objetivos
por asignar. 
...
distribución deseada existen $U$ segmentos que cumplen con la restricción de
distribución uniforme (\ref{ec:uniform}), existiendo $restoDist$ segmentos con
$cocienteDist+1=N$ robots asignados y $U-restoDist$ segmentos con
$cocienteDist$ robots asignados. El resto de segmentos tiene una cantidad de
objetivos menor a $cocienteDist$ y todos sus objetivos son asignados.

El \emph{calculo de valores} se lleva a cabo inicializando $cocienteDist$ y
$restoDist$ asumiendo una distribución completamente uniforme (líneas 5-6 y
9-10 primera iteración), sin considerar si es posible dado el número de
objetivos en cada segmento. Luego de forma iterativa, segmento a segmento y
comenzando por los que tienen menos objetivos (líneas 11-12), se comprueba si son
de los segmentos cuyos objetivos serán todos asignados (línea 13) y de serlo se
ajusta los valores para que la distribución sea uniforme en el resto de
conjuntos (líneas 14-17 y 9-10). Las iteraciones continúan hasta que solo
quedan los segmentos en los que van a quedar objetivos sin asignar (línea 8),
cuando esto sucede se dice que $cocienteDist$ y $restoDist$ convergen.
...
Dada función de despeje discretizada $\mli{DD} : \mli{CG} \rightarrow \mathds{R}$ que se
...
número par de celdas libres entre cada celda ocupada correspondiente a una
...
% al GVD según lau (solo se neceita para puntos criticos incluidos en el GVD). 
