...
conjnto de los identificadores de los objetivos valuados y $Costos$ un conjunto
de tuplas $(c_{i,j},r_i,o_j)$ los $c_{i,j}$ computados a partir de las
valuacion recibida de $r_i$ para $o_j$. El conjunto $S$ es el conjunto de
segmentos y la funcion $segmento : O \rightarrow S$ dado un objetivo devulve el
segmento al que pertence. Los conjuntos $O_1$,$O_2$,...,$O_k$,...,$O_{|S|}$ contienen los
...
%   // PARTE 1
%   //   Determinar el número a no superar mencionado en la restricción (2). Dicho número depende de dos dos números 
%   //   cocienteDistribucion y restoDistribucion, por lo tanto serán estos los que realmente se van a determinar.
  
    %ALT $segObjs :=$ Cola de prioridad vacia \tcp{Valores menores quedan primeros en la cola}
    $segObjs := \emptyset$\\

    %: cola de prioridad que contiene valores de tipo numFronteras (int) donde un menor numFronteras
    %  es prioritario frente a uno mayor. Inicializado vacío.

    % \tcp{DFS desde $c$ agregando las celdas visitadas a la componente conexa $C_i$}
  
%   /// recorrida de todos los elementos del diccionario, con el propósito de construir segFrontColaPrio
    \ForEach{$s_k \in S $}{ 
     %ALT segObjs.encolar($|O_k|$)\\
     $segObj = segObj \cup \{|O_k|\}$\\
    }
    ...
    \While{$\lnot (segObjs.vacia() \lor converge$)}{
...
%     // conciente crece siempre porque
    
%     // si no se puede parar es porque el segmento no tiene suficientes fronteras, por lo tanto se puede asumir que 
%     // que todas sus fronteras serán asignadas al intentar de distribuir de forma uniforme a los robots sobre los segmentos
%     // siendo la restricción (1) la limitante. Por lo tanto se calcula numRobots y numSeg teniendo lo anterior en cuenta
%     // (el segmento ya no se contabiliza en numSeg y se resta de numRobots tantos robots como fronteras tiene el segmento).
      \If{$\lnot converge$}{
        $numRobots := numRobots - segFront$\\
        $numSeg := numSeg - 1$\\
      }
    }
%   // PARTE 2
    ...
    $segRobNums :=$ diccionario de $S$ a enteros, siendo 0 el defecto\\   

    \While{$R \neq \emptyset \land O \neq \emptyset$}{
      $(c_{i,j}, r_i, o_j) := min\ Costos$ \tcp{Una tupla con minimo $c_{i,j}$}
...
El algorimo se puede dividir en dos partes, el calculo de valores
$cocienteDist$, $restoDist$ (lineas 1-18) y la asignacion distribuida (lineas
19-36). 
...
hacer $N$ rondas en las cuales se asigna un solo robot por segmento con tenga
...
objetivos estan todos asignados y se tienen segmentos dentro de los cuales
...
$cocienteDist$ robots asignados. La idea detras del calculo de $cocienteDist$ y
$restoDist$ es comenzar por una distribucion completamente uniforme sin
considerar los objetivos de los segmentos y luego de forma iterativa (lineas 8-18) ajustar
estos valores hasta sean consistentes con su definicion, es decir convergen.

Con los valores $cocienteDist$ y $restoDist$ calculados la asignacion conssite en...
...
En resumen, la central asigna a los robots de forma voraz y priorizando
distribuirlos lo mas posible sobre los segmentos dada la cantidad de objetivos
en cada segmento. Seguido a esto la central informa a cada robot su objetivo
asignado, quedando a la espera de nuevos pedidos por parte de los robots para
comenzar una nueva subasta. 

% El algorimo toma como entrada el conjunto de robots $r_i$ de los cuales se recibieron
% valuaciones, de objetivos $o_j$ valuados y de los costos calculados $c_{i,j}.
